...
- 🧱 **Per-device firewall.** A userspace firewall for mesh traffic, **separate from and layered on top of your host/kernel firewall** (iptables/nftables, pf, Windows Firewall) — both must allow a packet for it to reach a service, so a permissive host firewall does not open a port on the mesh, and this firewall does not touch your host rules. Directional, per-port, per-network rules with stateful return traffic. Secure by default: out of the box, unsolicited inbound TCP/UDP is denied (no local service port is exposed when you join a public network), while inbound ICMP (ping) and all outbound traffic are allowed. `ray firewall add in allow -p tcp --port N` opens a port (`--port` also takes a range `80-443` or a comma list `80,443`) — this is how you expose a service you run yourself, e.g. your host's own sshd on `--port 22`; `ray firewall default allow` restores permissive inbound behavior. Scope a rule to one peer with `--peer`, which accepts any peer identifier (a hostname, mesh IP, short id, or identity). By default a denied packet is silently dropped (stealthy); `ray firewall reject on` switches to "fail fast" mode, replying with a TCP RST / ICMP-unreachable so a blocked connection fails immediately ("connection refused") instead of hanging. For a simple setup that doesn't want a second firewall, `ray firewall off` disables enforcement entirely on that device (allow-all; `ray firewall on` restores it). `ray firewall show` lists your rules and posture.
