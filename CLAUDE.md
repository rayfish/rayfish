...
- **CLI help is grouped in `src/cli/help.rs`.** clap has no subcommand grouping, so the `-h` list is rendered there from the clap model and the template drops `{subcommands}`. A new command must join a `GROUPS` entry or it appears nowhere (tests enforce both directions). Subcommands stay *visible* in the clap model: `hide = true` would suppress clap's list but also make `clap_complete` skip them, gutting tab completion. Keep each `about` to one line inside 80 columns; the rest goes after a blank line, where it becomes `ray help <command>`.
