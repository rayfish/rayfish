...
- **`/etc/resolv.conf` is shared, not owned.** Another overlay's resolver in the file (`foreign_mesh_resolver`: a `100.64.0.0/10` nameserver that is not ours) means both daemons re-assert and the contents are whichever write landed last. That is only fatal while each write *replaces* the other, so ours does not: ours goes first (glibc stops at the first server that answers, so second place is no place), theirs stays as the next nameserver, and everything outside `.ray` is forwarded to it. `MERGE_COOLDOWN` damps the race instead of trying to win it. An additive write needs a subtractive undo, so every path that removes us (`restore_file`, the panic hook, `restore_stale_backups`) strips our own lines rather than restoring a backup that predates them. A change to the file's nameservers exits the watcher with a `Recapture` and re-runs detection: our forwarding upstreams came from that file, and re-rendering a departed VPN's address from a forwarder still aimed at it takes the host's DNS down (#111). The one thing merging cannot assume is that the resolver we forward to is not forwarding back (Tailscale drops only its *own* service IPs from what it reads, tailscale/tailscale#7816), so `LOOP_LIMIT` per name stops feeding the bounce and the stub falls through to their nameserver, which our render put second.
- **Search domains go wherever DNS actually lives.** They are what makes a bare `box` resolve, and `set_manager_search_domains` only speaks resolved — so on exactly the hosts that fall past resolved to a file-owning backend, they went nowhere. They are a `DnsConfigurator` method now: the split-DNS backends keep handing them to the manager, and the two backends that own a file write them into it (direct merges them under the host's own, resolvconf re-registers its stanza). Two things follow: the registry announces them before the data plane exists, so `DnsService` caches the list and re-installs it into whatever backend it adopts next (startup, a retry, a reclaim after standing down); and a captured `search` line is filtered of our own domains (`is_our_search_domain`), or a restart over our own file would readopt them as the host's and outlive the `ray leave` that dropped them.
