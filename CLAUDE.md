...
- **Config** lives under `config::config_dir()` (`/etc/rayfish` on Linux, `~/Library/Application Support/rayfish` on macOS, i.e. the *daemon's* home, `/var/root` under launchd; `RAYFISH_CONFIG_DIR` overrides it on every platform, and daemon + CLI must both see the same value): sharded + atomic: globals in `settings.toml`, one network per `networks/<name>.toml`. Secret-bearing files are `0600 root:root`; writes go through `config::write_file` (temp file + rename).
