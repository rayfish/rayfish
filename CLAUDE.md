...
- `src/identity.rs` — persistent Ed25519 keypair at `~/.config/pitopi/secret_key`; device certificate support: `create_device_cert()` generates a certificate signed by the user identity key, `store_device_cert()` and `load_device_cert()` persist device certs to `~/.config/pitopi/device_cert`
...
**Pairing (multi-device identity):** Primary device runs `pitopi pair` → daemon generates a pairing secret, creates a pairing ticket (`bs58(endpoint_id || pairing_secret)`), displays it as text and QR code (via `qr2term`), and registers a handler on PAIR_ALPN (`pitopi/pair/1`). Secondary device runs `pitopi pair <ticket>` → daemon decodes the ticket, connects to the primary via PAIR_ALPN, authenticates with the pairing secret. Primary signs a `DeviceCert` binding the secondary's transport key to the primary's user identity, sends it back. Secondary stores the device cert at `~/.config/pitopi/device_cert`. On subsequent network joins, the secondary presents its device cert in MeshHello — peers verify the cert signature against the user identity and add a `DeviceUserMap` entry (secondary transport key → user identity). ACL tags and rules referencing the user identity automatically cover all paired devices. `pitopi pair backup` encrypts the identity key with a user-provided passphrase (argon2 KDF + chacha20poly1305) and displays a backup code. `pitopi pair restore <backup-code>` decrypts and restores the identity key.
...
- Device certificate persists to `~/.config/pitopi/device_cert`
- Pairing ALPN: `pitopi/pair/1`
