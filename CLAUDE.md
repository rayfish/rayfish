...
- **Owning `/etc/resolv.conf` is a standing verdict, not a startup one.** The direct backend refuses the file when another overlay's resolver is in it (`foreign_mesh_resolver`: a `100.64.0.0/10` nameserver that is not ours can only be another magic DNS, no vendor marker involved), because both sides re-assert and the host's DNS ends up being whichever write landed last. Deciding that once, at `apply`, only covers losing the startup race: hold the file first and the re-assert watcher fights the VPN that arrives second, which is the same war from the other side. So `reassert_decision` runs the check on every pass and the watcher **stands down** (`DnsService::stand_down`) rather than rewrite. Standing down is not `revert`: our backup is the host from before either VPN, so restoring it would break theirs. Drop the backup, keep the `dns=none` drop-in (un-quieting NM only starts a different fight), and hand back to `spawn_configure_retry`, which reclaims DNS when the other VPN leaves. **The magic IP still answers throughout** (`is_magic_dns` intercepts in the TUN read loop, not via resolv.conf); what is lost is only the system stub path.
