...
- **A full tunnel takes only the families its own data plane carries.** IPv6-only mode tunnels IPv6 and leaves the host's IPv4 egress where it was, with whoever owns `100.64.0.0/10` there. That is not a limitation worked around, it is the same premise the mode rests on: mesh IPv4 carries no traffic, so claiming the host's IPv4 default would source transit from a `/32` this mode deliberately leaves unrouted. It also keeps `is_disabled_mesh_ipv4` out of it, since exit return traffic is then IPv6 and never meets that rule. **Install is per-family and also tears down the family it stopped claiming; teardown is never per-family** (`tunnel_families`), because kernel rules outlive the process and the panic hook `abort()`s, so neither end may assume the other ran. The gateway side was always dual-stack and is untouched. Two things follow: a gateway needs its own IPv6 uplink, so the offer carries `Member.exit_node_v6` and selecting one without it is refused rather than black-holed; and the daemon's DNS forwarder is pointed at an IPv6 resolver for the tunnel's lifetime (`exit_node::tunnel_upstreams`), because every upstream the OS capture yields is IPv4 and would otherwise leave *around* the exit that the traffic goes through.
- **That IPv6 gate is a standing property, not a selection-time one, and the two halves of the refusal are split for that reason.** "Does not advertise an exit node" flickers (a coordinator rebuild, a gateway mid-restart), and dropping a live tunnel on it would leak the traffic the user chose to tunnel, so it is only checked when the user picks. "Cannot carry IPv6" is checked again on every re-apply (`ipv6_gateway_refusal` in `reload_exit_state`), because while it holds the tunnel carries nothing at all, and nobody has to touch the selection for it to become true: `ipv6_only = auto` turns the mode on by itself the first time something else claims `100.64.0.0/10`. There the safe side is *not* installing, the opposite of the no-silent-fallback rule beside it. For the same reason the advertised flag is re-probed on the reconverge that publishes it, not just at `ray up`, or a gateway that gained IPv6 would keep saying otherwise until someone restarted its data plane.
- **Pointing our forwarder at an IPv6 resolver is not the same as an application's lookups following the tunnel.** Which queries reach the daemon at all is the OS DNS backend's decision. Only the direct-`resolv.conf` backend hands us everything; the split-DNS ones (systemd-resolved, NetworkManager, resolvconf) register `~ray` as the sole routing domain, so in IPv6-only mode every other lookup goes to the host's other servers over the IPv4 this mode does not tunnel, and leaves around the exit. macOS avoids it by re-asserting a catch-all match domain while the tunnel is up (`DnsService::reassert_os_config`); Linux has no equivalent yet and the daemon warns instead. Don't restate the macOS behaviour as if it were the general case.
- **Our policy rules outrank a peer VPN's, so we mirror what they would have matched.** `PREF_TUNNEL` is a catch-all far above the preferences a co-resident VPN uses (Tailscale's are 5210-5270), and `PREF_MAIN`'s `suppress_prefixlength 0` only rescues routes in `main` — a policy-routing VPN keeps its prefixes in a private table. So the client install copies every foreign table's non-default routes into `EXIT_TABLE` first (`mirror_foreign_routes`), where longest-prefix hands those destinations back to the interface that owned them. One-directional by construction: we only ever write our own table, and the teardown flush drops the copies with the default. macOS needs none of this, because its split default lives in the one table a foreign route is already more specific than.
